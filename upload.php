    ...
    else 
    {
        ...
        for($row = 3; $row <= $lastRow; $row++)
        {
            ...
            {  
                ...
                $PROF1 = $sheet->getCell('G'.$row)->getValue();
                $PROF2 = $sheet->getCell('H'.$row)->getValue();
                $PROF3 = $sheet->getCell('I'.$row)->getValue();
                $PROF4 = $sheet->getCell('J'.$row)->getValue();
                $PROF5 = $sheet->getCell('K'.$row)->getValue();     //與會老師們

                // TODO: prepare statement
                
                $get_prof_teaching= explode("、", $PROF); //分割指導老師 回傳陣列
                //print_r ($get_prof_teaching);
                $arr = [];                                //ID存進陣列
                ...
                while ($row3=mysqli_fetch_row($get_stu_id))         //當有抓到學生資料時
                {
                    ...
                    for($colum = 6;$colum<=10;$colum++)
                    {
                        while(trim($sheet->getCellByColumnAndRow($colum, $row)->getValue())!="")
                        {                            
                            array_push($row_prof_id,$sheet->getCell($colum.$row)->getValue());
                        }
                    }
                    var_dump($row_prof_id);
                    $row_stu_id = $row3[0];
                    //echo "學生ID:".$row3[0]."<br>";
                    //echo "學生學號:".$row3[1]."<br>";
